def run(x):
    x = x.split("\n")
    print(sum(map(is_valid, x)))
    print(sum(map(is_valid2, x)))


def is_valid(x):
    i, j, char, password = input(x)
    count = 0
    for y in password:
        if y == char:
            count += 1
    return count >= i and count <= j


def is_valid2(x):
    i, j, char, password = input(x)
    return bool(password[i - 1] == char) ^ bool(password[j - 1] == char)


def input(x):
    x = x.split(" ")
    nums = x[0].split("-")
    return int(nums[0]), int(nums[1]), x[1][0], x[-1]


if __name__ == "__main__":
    x = """6-10 s: snkscgszxsssscss
6-7 b: bbbbbxkb
2-4 n: nnnjn
1-2 j: jjjj
5-9 z: jgzzzqhbj
