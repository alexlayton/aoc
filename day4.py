def run(x):
    print(passports(x, is_valid))
    print(passports(x, is_valid2))


def passports(x, valid_func):
    def to_dict(y):
        y = map(lambda z: z.split(":"), y.split())
        return {k: v for k, v in y}

    x = map(to_dict, x.split("\n\n"))
    x = map(valid_func, x)
    return sum(x)
...
def valid_byr(x):
    return int_between(x, 1920, 2002)


def valid_iyr(x):
    return int_between(x, 2010, 2020)


def valid_eyr(x):
    return int_between(x, 2020, 2030)
...
def valid_hcl(x):
    return x[0] == "#" and str(x[1:]).isalnum()


def valid_ecl(x):
    return x in set(["amb", "blu", "brn", "gry", "grn", "hzl", "oth"])
...
if __name__ == "__main__":
    x = """byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
