def run(x):
    x = x.split("\n")
    rules = dict(map(parse, x))

    bags = filter(lambda x: x != "shiny gold", rules.keys())
    contains_shiny_gold = sum(map(lambda x: has_shiny_gold(x, rules), bags))
    print(contains_shiny_gold)

    print(bag_count(rules["shiny gold"], rules))


def has_shiny_gold(x, rules):
    if x == "shiny gold":
        return True
    else:
        for y in rules[x].keys():
            if has_shiny_gold(y, rules):
                return True
        return False


def bag_count(x, rules):
    if not x:
        return 0
    else:
        count = 0
        for k, v in x.items():
            count += v * (1 + bag_count(rules[k], rules))
        return count
...
def parse(x):
    ...
    if x[1] == "no other bags.":
        ...
        # Remove bag/bags
        values = map(lambda x: x.replace("bags", "bag").replace(" bag", ""), values)

        # map to key value pairs
        values = map(lambda x: (x[2:], int(x[0])), values)

        return (key, dict(values))


if __name__ == "__main__":
    x = """vibrant bronze bags contain 3 dim olive bags.
